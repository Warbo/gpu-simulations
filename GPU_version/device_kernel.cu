// RIPPED DIRECTLY FROM NVIDIA'S N-BODY EXAMPLE. ONLY USE THIS TO
// EXPERIMENT, WRITE FROM SCRATCH WHEN WE UNDERSTAND HOW TO USE IT!
...
#ifndef _GRID_KERNEL_H_
#define _GRID_KERNEL_H_
...
__device__ void particle_interaction(particle* p1, particle* p2) {
	/*
	 * Computes the interaction between a pair of particles.
	 * Just does gravity as a placeholder for now.
	 */
	p1.x_acc += 6.67e-11 * (p2.x - p1.x);
	p1.y_acc += 6.67e-11 * (p2.y - p1.y);
	p1.z_acc += 6.67e-11 * (p2.z - p1.z);
}
...
__device__ void calculate_one_particle(particle* current_particle) {
	/*
	 * Applies the particle_interaction calculation between the given
	 * particle and all of its potential neighbours.
	 */
	// Would we be better off keeping the varying properties separate
	// to the read-only properties? For example put mass, position, etc.
	// into a float array current_particle and acceleration, etc. into
	// a float array another_argument?
	
	// Seems our array index is best off as long for efficiency on
	// 64bit systems (otherwise unneccessary overflow handling happens),
	// so we keep our loop index separate.
	long index = 0;
	
	// Need to define buffer_size (shouldn't hurt to do buffer_size
	// calculations even if the number of potential neighbours is less
	// than buffer_size)
	// Remember to counter++!
	// Unrolling loops should increase efficiency (ie. do several
	// iterations per loop), but premature optimisation is the root of
	// all evil so leave it for now.
	for (unsigned int counter = 0; counter < buffer_size; ) {
		particle_interaction(current_particle, buffer[index++]);
		counter++;
	}
}
